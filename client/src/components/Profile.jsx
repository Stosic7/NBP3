...
const Profile = ({ darkMode, user, onLogout }) => {
  ...
  const stats = [
    ...
    {
      label: "Dodati Uređaji",
      value: user.devicesCount || 0,
      icon: Package,
      color: "text-blue-500",
      bg: "bg-blue-500/10",
    },
    {
      label: "Trenutni Rank",
      value: safePoints > 500 ? "Expert" : "Novice",
      icon: Award,
      ...
      >
        <div className="h-64 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-500 relative">
          ...
          <div className="absolute -bottom-16 left-12 flex items-end gap-6">
            ...
            <div className="mb-4">
              <h2
                className={`text-5xl font-black mb-2 drop-shadow-lg ${darkMode ? "text-white" : "text-white"}`}
              >
                {user.name}
              </h2>
              <div className="flex items-center gap-3 text-white/90 font-medium">
                ...
                <span className="flex items-center gap-1 bg-white/20 backdrop-blur-md px-3 py-1 rounded-full text-sm">
                  ...
                  {safePoints > 500 ? "Top 5%" : "Novi Član"}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="pt-24 px-12 pb-12">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {stats.map((stat, index) => (
              ...
              >
                ...
                <div
                  className={`text-sm font-bold mb-1 ${darkMode ? "text-slate-400" : "text-slate-500"}`}
                >
                  {stat.label}
                </div>
                <div
          ...
          <div
            className={`p-8 rounded-[2rem] mb-12 ${darkMode ? "bg-slate-900/50" : "bg-blue-50/50"}`}
          >
            <div className="flex justify-between items-center mb-4">
              <span
                className={`font-bold ${darkMode ? "text-white" : "text-slate-900"}`}
              >
                Progres do sledećeg nivoa
              </span>
            ...
            <div
              className={`h-4 w-full rounded-full overflow-hidden ${darkMode ? "bg-slate-700" : "bg-slate-200"}`}
            >
              <motion.div
                initial={{ width: 0 }}
            ...
            <p
              className={`mt-4 text-sm font-medium ${darkMode ? "text-slate-400" : "text-slate-500"}`}
            >
              Još {experienceToNextLevel - safePoints} bodova do Levela{" "}
              {Math.floor(safePoints / 1000) + 2}!
            </p>
          </div>
